...
public:
    ...
    bool PMDijkstra(uint32_t rank, uint32_t V, vector<vector<pair<uint32_t , float > > > adj_out,
 vector<vector<pair<uint32_t , float > > > adj_in, vector<pair<vector<uint32_t >, vector<float > > > *tmp_index_in,
 vector<pair<vector<uint32_t >, vector<float > > > *tmp_index_out);
...
bool WidestPath
::PMDijkstra(uint32_t rank, uint32_t V, vector<vector<pair<uint32_t , float > > > adj_out,
 vector<vector<pair<uint32_t , float > > > adj_in, vector<pair<vector<uint32_t >, vector<float > > > *tmp_index_in1,
 vector<pair<vector<uint32_t >, vector<float > > > *tmp_index_out1){
...
::constructIndex(const vector<tuple<uint32_t ,uint32_t ,float > > &es){
    ...
    SYSTEM_INFO si;
    GetSystemInfo(&si);
    uint32_t KeyNum = si.dwNumberOfProcessors;
    thread tid[KeyNum];
    ...
    for (int i = 0; i < KeyNum; ++i)
    {
        threads.emplace_back([&, this](){
            PMDijkstra(KeyNum,V,adj_out,adj_in,&tmp_index_in,&tmp_index_out);
        });
    }
    for (int i = 0; i < KeyNum; ++i)
    {
